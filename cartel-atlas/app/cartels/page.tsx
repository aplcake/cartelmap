...
function CartelCard({ cartel, onClick }: { cartel: Cartel; onClick: () => void }) {
  const color = CARTEL_COLORS[cartel.id] || '#555';
  const status = STATUS_STYLE[cartel.status];
    ...
    <div style={{ background: '#0a0a16', minHeight: '100vh', color: '#fff', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ padding: '24px 40px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {(['active', 'fragmented', 'dissolved'] as const).map(s => (
              <button key={s} onClick={() => setStatusFilter(f => f === s ? null : s)} style={{
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            {eras.map(era => (
              <button key={era} onClick={() => setEraFilter(f => f === era ? null : era)} style={{
        ...
        <div style={{
          ...
          {filtered.map(c => (
            <CartelCard key={c.id} cartel={c} onClick={() => setSelected(c.id)} />
          ))}
        </div>
