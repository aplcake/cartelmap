  ...
  for (const m of matches) {
    ...
    parts.push(
      <span
        ...
        onClick={e => { e.stopPropagation(); onSelect(m.person); }}
        ...
        onMouseLeave={e => { e.currentTarget.style.background = `${col}25`; }}
      >👤 {m.term}</span>
    );
    pos = m.end;
  }
...
// ── PersonCard ───────────────────────────────────────────
function PersonCard({ person, onSelect, selected }: { person: Person; onSelect:(p:Person)=>void; selected:boolean }) {
          ...
          {CARTELS.filter(c => NODE_POSITIONS[c.id]).map(c => {
            ...
            return (
              <g key={c.id}
                ...
                onClick={()=>setSelectedCartel(s => s===c.id ? null : c.id)}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 360px',height:'calc(100vh - 49px)'}}>
          <div style={{overflowY:'auto',padding:20}}>
            <div style={{display:'flex',gap:12,marginBottom:16,alignItems:'center'}}>
              <input placeholder="Search name or alias…" value={search} onChange={e=>setSearch(e.target.value)}
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(190px,1fr))',gap:10}}>
              {filteredPeople.map(p=><PersonCard key={p.id} person={p} onSelect={setSelectedPerson} selected={selectedPerson?.id===p.id}/>)}
            </div>
          </div>
          <div style={{borderLeft:'1px solid #222',overflowY:'auto',padding:14,background:'#0a0a16'}}>
            {selectedPerson ? <PersonDetail person={selectedPerson} onSelect={setSelectedPerson}/> : (
