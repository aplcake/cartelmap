...
export default function MapPage() {
  ...
  const activeWars     = useMemo(() => CARTEL_WARS.filter(w => w.startYear <= year && (!w.endYear || w.endYear >= year)), [year]);
  const activeHotspots = useMemo(() => VIOLENCE_HOTSPOTS.filter(h => h.startYear <= year && h.endYear >= year), [year]);
  const activeRoutes   = useMemo(() => TRAFFICKING_ROUTES.filter(r => r.startYear <= year && r.endYear >= year), [year]);
  ...
  function getStateInfo(code: string): string {
    if (layerMode === 'hotspots') {
      const spots = activeHotspots.filter(h => h.stateCode === code);
  ...
  const mapWars    = layerMode === 'wars'    ? (cf ? activeWars.filter(w => w.cartel1===cf||w.cartel2===cf) : activeWars) : [];
  const mapRoutes  = layerMode === 'routes'  ? (cf ? activeRoutes.filter(r => r.cartelId===cf) : activeRoutes) : [];
  ...
  return (
    ...
    <div style={{background:'#0a0a16',minHeight:'100vh',color:'#fff',fontFamily:'system-ui,sans-serif',display:'flex',flexDirection:'column',height:'100vh',overflow:'hidden'}}>
      ...
      <div style={{background:'#0d0d1d',borderBottom:'1px solid #1f1f30',padding:'8px 16px',display:'flex',alignItems:'center',gap:14,flexShrink:0,flexWrap:'wrap'}}>
        ...
        {/* Play / Pause */}
        <button
          onClick={() => { if (year >= 2026) { setYear(1930); setPlaying(true); } else setPlaying(p => !p); }}
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 272px',flex:1,overflow:'hidden',minHeight:0}}>
        ...
        <div style={{position:'relative',background:'#0a0a16',overflow:'hidden'}}>
          <MexicoMap
            ...
            onAttackClick={a => { setSelectedAttack(x => x?.id===a.id?null:a); setSelectedSite(null); setSelectedBust(null); setSelectedWar(null); setSelectedState(null); }}
            onSiteClick={s => { setSelectedSite(x => x?.id===s.id?null:s); setSelectedAttack(null); setSelectedBust(null); setSelectedWar(null); setSelectedState(null); }}
            onBustClick={b => { setSelectedBust(x => x?.id===b.id?null:b); setSelectedAttack(null); setSelectedSite(null); setSelectedWar(null); setSelectedState(null); }}
            onWarClick={w => { setSelectedWar(x => x?.id===w.id?null:w); setSelectedAttack(null); setSelectedSite(null); setSelectedBust(null); setSelectedState(null); }}
            selectedRoute={selectedRoute}
            onRouteClick={r => setSelectedRoute(x => x===r.id ? null : r.id)}
            selectedHotspot={selectedHotspot}
            onHotspotClick={h => {
              setSelectedHotspot(x => x===h.id ? null : h.id);
          ...
          {(selectedState || selectedAttack || selectedSite || selectedBust || selectedWar) ? (
            <div style={{flex:1,overflowY:'auto'}}>
                        ...
                        <div>
                          ...
                          {people.map((p:any) => (
                            <div key={p.id}
                              ...
                              onMouseEnter={e=>(e.currentTarget.style.borderColor='#C8282D55')}
              ...
              {selectedBust && (() => {
                ...
                const totalKg = b.drugs?.reduce((s,d)=>s+d.kg,0)||0;
                const totalVal = b.drugs?.reduce((s,d)=>s+(d.streetValueUSD||0),0)||0;
                return (
                  <div>
                    ...
                    {w.states?.length > 0 && (
                      <div style={{marginTop:8,marginBottom:12}}>
                        ...
                        <div style={{display:'flex',flexWrap:'wrap',gap:4}}>
                          {w.states.map(s => <span key={s} style={{background:'#111',border:'1px solid #222',borderRadius:3,padding:'2px 6px',fontSize:10,color:'#666'}}>{s}</span>)}
                        </div>
                      </div>
                    )}
                    {w.personIds?.length > 0 && (() => {
            ...
            <div style={{padding:'12px 14px',flex:1,overflowY:'auto'}}>
              ...
              {layerMode==='attacks' && activeAttacks.length>0 && (
                <div style={{marginBottom:14}}>
                  {activeAttacks
                    .slice().sort((a,b) => (b.killed||0)-(a.killed||0))
                    .map(a => {
              ...
              {layerMode==='sites' && activeSites.length>0 && (
                <div style={{marginBottom:14}}>
                  {activeSites
                    .slice().sort((a,b)=>(b.victims||0)-(a.victims||0))
                    .map(s => (
              ...
              {layerMode==='busts' && activeBusts.length>0 && (
                <div style={{marginBottom:14}}>
                  {activeBusts
                    .slice().sort((a,b)=>{
                      const akg = a.drugs?.reduce((s,d)=>s+d.kg,0)||0;
                      const bkg = b.drugs?.reduce((s,d)=>s+d.kg,0)||0;
                      return bkg-akg;
                    })
                    .map(bust => {
                      const kg = bust.drugs?.reduce((s,d)=>s+d.kg,0)||0;
                      const val = bust.drugs?.reduce((s,d)=>s+(d.streetValueUSD||0),0)||0;
                      return (
              ...
              {layerMode==='wars' && activeWars.length>0 && (
                <div style={{marginBottom:14}}>
                  {activeWars.map(w => {
                    const c1 = CARTELS.find(c=>c.id===w.cartel1);
                    const c2 = CARTELS.find(c=>c.id===w.cartel2);
                    const isSelected = selectedWar?.id === w.id;
                    return (
                      <div key={w.id} onClick={()=>{setSelectedWar(x=>x?.id===w.id?null:w);setSelectedAttack(null);setSelectedSite(null);setSelectedBust(null);setSelectedState(null);if(w.lat&&w.lng)mapRef.current?.flyTo(w.lat,w.lng,6);}}
              ...
              {layerMode==='hotspots' && activeHotspots.length>0 && (
                <div ref={hotspotListRef} style={{marginBottom:14}}>
                  {activeHotspots.sort((a,b)=>b.intensity-a.intensity).map(h => (
                    <div
                      key={h.id}
                      id={`hotspot-${h.id}`}
                      onClick={() => {
              ...
              {layerMode==='routes' && activeRoutes.length>0 && (
                <div style={{marginBottom:14}}>
                  {activeRoutes.map(r => {
                    ...
                    return (
                      <div key={r.id}
                        onClick={() => setSelectedRoute(x => x===r.id ? null : r.id)}
              ...
              <div style={{marginTop:8}}>
                ...
                <div style={{display:'flex',flexWrap:'wrap',gap:3,marginBottom:14}}>
                  ...
                  {activeCartels.map(c => (
                    <button key={c.id} onClick={()=>setSelectedCartelFilter(f=>f===c.id?null:c.id)}
